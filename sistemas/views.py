...
def gauss_pivoteo_parcial(A, b):
    ...
    for k in range(n - 1):  # Iterar sobre las filas de la matriz
        # Pivoteo Parcial
        max_row = (
            np.argmax(abs(Ab[k:, k])) + k
        )  # Fila con el pivote más grande (absoluto)
        ...
        for i in range(k + 1, n):  # Iterar sobre las filas debajo de la actual
            factor = (
                Ab[i, k] / Ab[k, k]
            )  # Factor para hacer cero el elemento debajo del pivote
            Ab[i, k:] -= (
                factor * Ab[k, k:]
            )  # Restar la fila actual multiplicada por el factor
    ...
    for i in range(n - 1, -1, -1):  # Iterar desde la última fila hasta la primera
        x[i] = (Ab[i, -1] - np.dot(Ab[i, i + 1 : n], x[i + 1 :])) / Ab[
            i, i
...
def gauss_pivoteo_total(A, b):
    ...
    for k in range(n - 1):  # Iterar sobre las filas de la matriz
        ...
        max_row, max_col = np.unravel_index(
            np.argmax(submat), submat.shape
        )  # Fila y columna del pivote
...
def factorizacion_lu_doolittle(A):
    ...
    for i in range(n):
        ...
        for j in range(i, n):
            U[i, j] = A[i, j] - sum(
                L[i, k] * U[k, j] for k in range(i)
            )  # U[i, j] = A[i, j] - sum(L[i, k] * U[k, j]) basicamente filas por columnas de la matriz A menos la suma de las filas por columnas de la matriz L por la matriz U

        # Calcular elementos de L
        for j in range(i + 1, n):
            L[j, i] = (A[j, i] - sum(L[j, k] * U[k, i] for k in range(i))) / U[
                i, i
            ]  # L[j, i] = (A[j, i] - sum(L[j, k] * U[k, i])) / U[i, i] basicamente filas por columnas de la matriz A menos la suma de las filas por columnas de la matriz L por la matriz U dividido por la matriz U
...
def jacobi(request):
    if request.POST:
        ...
        x, iteraciones, error_msg, alert, error_abs, error_rel, grafica = metodo_jacobi(
            A, b, x0, tol, 100
        )
...
def metodo_jacobi(A, b, x0, tol, max_iter):
    ...
    for k in range(max_iter):
        x_new = np.zeros_like(
            x
        )  # Crear un vector de ceros con la misma forma que x, es decir, n elementos para los valores del sistema de ecuaciones

        for i in range(n):  # Iterar sobre las filas de la matriz
            suma = sum(
                A[i, j] * x[j] for j in range(n) if j != i
            )  # Sumar los elementos de la fila actual, excepto el de la diagonal principal
        ...
        if (
            error_abs < tol
        ):  # Calcular la norma infinito de la diferencia entre los vectores x_new y x, la norma infinito es el máximo valor absoluto de los elementos del vector, es decir el error máximo
            alert = "success"
            error_msg = "El sistema se resolvió correctamente."
    ...
    error_msg = "Error: El método no converge después de {} iteraciones.".format(
        max_iter
    )
    alert = "danger"
    graph_base64 = graficar_iteraciones(valores_x)
...
def graficar_iteraciones(valores_x):    #Para Jacobi y Gauss-Seidel
    """
    Genera una gráfica de las iteraciones y la devuelve como una cadena Base64.
    ...
    Retorna:
        str:
            Gráfica en formato Base64.
    """
    plt.figure(figsize=(10, 6))
    for i in range(len(valores_x[0])):
        plt.plot([val[i] for val in valores_x], label=f'x[{i}]')
